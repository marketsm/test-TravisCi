	...
	stages {
		...
		stage ('studentsTests'){
			steps {
				...
				sh 'perl collectResults.pl vortagConfig.xml getTestCoverage'
				sh 'perl collectResults.pl vortagConfig.xml switchClasses "mvn -B verify -Dmaven.test.failure.ignore -Dmaven.compiler.showDeprecation -Dmaven.compiler.showWarnings"'			
			}
		}
		
		stage ('teachersTests'){
			steps {
				sh 'perl collectResults.pl vortagConfig.xml addTeachersTests'
				...
				sh 'perl collectResults.pl vortagConfig.xml'
			}
		}
