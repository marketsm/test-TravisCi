	...
	stages {
		...
		stage ('studentsTests'){
			steps {
				...
				sh 'perl collectResults.pl getTestCoverage'
				sh 'perl collectResults.pl switchClasses "mvn -B verify -Dmaven.test.failure.ignore -Dmaven.compiler.showDeprecation -Dmaven.compiler.showWarnings"'			
			}
		}
		
		stage ('teachersTests'){
			steps {
				sh 'perl collectResults.pl addTeachersTests'
